...
function Text1({ userEmail }: { userEmail?: string }) {
  const nik = userEmail === "binavia@injourney.co.id" ? "26010002" : "26010001";
  
  return (
    <div className="h-[16.5px] relative shrink-0 w-[52.227px]" data-name="Text">
      <p className="absolute font-['Inter:Semi_Bold',sans-serif] font-semibold leading-[16.5px] left-[26px] not-italic text-[#181d27] text-[11px] text-center text-nowrap top-[0.5px] translate-x-[-50%] whitespace-pre">{nik}</p>
    </div>
  );
}

function Container({ userEmail }: { userEmail?: string }) {
  return (
    <div className="bg-neutral-100 box-border content-stretch flex gap-[8px] items-center px-[12px] py-[4px] relative rounded-[1.67772e+07px] shrink-0" data-name="Container">
      <Text />
      <Text1 userEmail={userEmail} />
    </div>
  );
}

function Container1({ userEmail }: { userEmail?: string }) {
  ...
  return (
    <div className="content-stretch flex flex-col gap-[4px] items-start justify-center relative shrink-0" data-name="Container">
      <p className="font-['Inter:Semi_Bold',sans-serif] font-semibold leading-[24px] not-italic relative shrink-0 text-[#181d27] text-[16px] text-center text-nowrap whitespace-pre">{userName}</p>
      <p className="font-['Inter:Medium',sans-serif] font-medium leading-[18px] not-italic relative shrink-0 text-[#181d27] text-[12px] text-nowrap whitespace-pre">{jobTitle}</p>
      <Container userEmail={userEmail} />
    </div>
  );
}
...
function Frame({ userEmail }: { userEmail?: string }) {
  return (
    <div className="content-stretch flex gap-[16px] items-center relative shrink-0 w-full">
      ...
      <Container1 userEmail={userEmail} />
    </div>
  );
}
...
function Container6({ userEmail }: { userEmail?: string }) {
  const email = userEmail === "binavia@injourney.co.id" ? "binavia@injourney.co.id" : "dimas.sayyid@injourney.id";
  ...
  return (
    <div className="basis-0 grow h-[33px] min-h-px min-w-px relative shrink-0" data-name="Container">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border content-stretch flex flex-col h-[33px] items-start not-italic relative text-nowrap w-full whitespace-pre">
        ...
        <p className="font-['Inter:Medium',sans-serif] font-medium leading-[18px] relative shrink-0 text-[#181d27] text-[12px]">{email}</p>
      </div>
    </div>
  );
}

function Container7({ userEmail }: { userEmail?: string }) {
  return (
    <div className="bg-[rgba(245,245,245,0.5)] relative rounded-[8px] shrink-0 w-full" data-name="Container">
      <div className="size-full">
        <div className="box-border content-stretch flex gap-[12px] items-start p-[12px] relative w-full">
          <Container5 />
          <Container6 userEmail={userEmail} />
        </div>
      </div>
    </div>
  );
...
function Container9({ userEmail }: { userEmail?: string }) {
  ...
  return (
    <div className="basis-0 grow min-h-px min-w-px relative shrink-0" data-name="Container">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border content-stretch flex flex-col items-start not-italic relative text-nowrap w-full whitespace-pre">
        ...
        <p className="font-['Inter:Medium',sans-serif] font-medium leading-[18px] relative shrink-0 text-[#181d27] text-[12px]">{directSupervisor}</p>
        {userEmail !== "binavia@injourney.co.id" && (
          <p className="font-['Inter:Regular',sans-serif] font-normal leading-[16.5px] relative shrink-0 text-[#717680] text-[11px]">{supervisorTitle}</p>
        )}
      </div>
    </div>
  );
...
function Container10({ userEmail }: { userEmail?: string }) {
  return (
    <div className="bg-[rgba(245,245,245,0.5)] relative rounded-[8px] shrink-0 w-full" data-name="Container">
      <div className="size-full">
        <div className="box-border content-stretch flex gap-[12px] items-start p-[12px] relative w-full">
          <Container8 />
          <Container9 userEmail={userEmail} />
        </div>
      </div>
    </div>
  );
...
function Container11({ userEmail }: { userEmail?: string }) {
  return (
    <div className="content-stretch flex flex-col gap-[12px] items-start relative shrink-0 w-full" data-name="Container">
      <Container4 />
      <Container7 userEmail={userEmail} />
      <Container10 userEmail={userEmail} />
    </div>
  );
}
...
function Container12({ onProfileClick, onLogout, userEmail }: { onProfileClick?: () => void; onLogout?: () => void; userEmail?: string }) {
  return (
    <div className="box-border content-stretch flex flex-col gap-[16px] items-center p-[16px] relative shrink-0" data-name="Container">
      ...
      <Container11 userEmail={userEmail} />
