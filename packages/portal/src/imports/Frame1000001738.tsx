...
import { OnboardingDashboardSection } from "../components/onboarding/OnboardingDashboardSection";
...
function AnnouncementsSectionBackgroundImage({ children }: React.PropsWithChildren<{}>) {
  return (
    <div className="relative shrink-0 w-full min-h-[105.328px]">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid content-stretch flex gap-[16px] items-start relative w-full">{children}</div>
    </div>
  );
}
...
function BackgroundImage13({ children }: React.PropsWithChildren<{}>) {
  return (
    <div className="basis-0 grow min-h-[105.328px] min-w-px shrink-0">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid flex flex-col w-full">{children}</div>
    </div>
  );
}
...
function ExternalSystemsBackgroundImage({ children }: React.PropsWithChildren<{}>) {
  return (
    <div className="relative shrink-0 w-full" style={{ backgroundColor: '#fafafa', borderRadius: 'var(--radius)' }}>
...
function AnnouncementSectionBackgroundImage({ children }: React.PropsWithChildren<{}>) {
  return (
    <div className="relative shrink-0 w-full" style={{ backgroundColor: 'white', borderRadius: 'var(--radius)', border: '1px solid var(--border)' }}>
      <div className="size-full">
        <div className="content-stretch flex flex-col gap-[16px] items-start p-[24px] relative w-full">{children}</div>
      </div>
    </div>
...
export default function Frame({ onProfileClick, userRole = "Admin", userEmail = "dimas@injourney.co.id" }: { onProfileClick?: () => void; userRole?: "Admin" | "User"; userEmail?: string }) {
  ...
  const jobTitle = userEmail === "binavia@injourney.co.id" ? "HC Strategy Senior Officer" : "VP Human Capital Strategy";
  ...
  const [announcementView, setAnnouncementView] = useState<"list" | "carousel">("list");
  const [currentPage, setCurrentPage] = useState(1);
  const [announcementFilter, setAnnouncementFilter] = useState<'all' | 'learning' | 'talent' | 'performance' | 'ai' | 'general'>('all');
  const [selectedAnnouncement, setSelectedAnnouncement] = useState<any | null>(null);
  const itemsPerPage = 10;
  
  // EQS Detail Modal state
  const [isEQSModalOpen, setIsEQSModalOpen] = useState(false);
  
  // External Systems - show more state
  const [showAllSystems, setShowAllSystems] = useState(false);
  
  // Offboarding state - check if current user is in offboarding process
  const isUserOffboarding = userEmail === "binavia@injourney.co.id"; // Demo: binavia is offboarding
  const [showOffboardingModal, setShowOffboardingModal] = useState(false);

  // Load announcements from localStorage
  useEffect(() => {
    const loadedAnnouncements = announcementStorage.getActive();
  ...
  const filteredAnnouncements = announcementFilter === 'all' 
    ? announcements 
    : announcements.filter(a => a.iconType === announcementFilter);
  
  // For dashboard display: Show only 4 announcements with 4 different types
  // Keep the 2 newest announcements, then add up to 2 more with different types
  const getDashboardAnnouncements = () => {
    if (announcementFilter !== 'all') {
      // If filtered, use regular pagination
      const startIndex = (currentPage - 1) * itemsPerPage;
      const endIndex = startIndex + itemsPerPage;
      return filteredAnnouncements.slice(startIndex, endIndex);
    }
    
    // For 'all' filter: limit to 4 with different types
    const result = [];
    ...
    // Take max 4 announcements with different types
    for (const announcement of sortedAnnouncements) {
      if (result.length >= 4) break;
      
      const type = announcement.iconType || 'general';
    ...
    // If we have less than 4, add more from newest regardless of type
    if (result.length < 4) {
      for (const announcement of sortedAnnouncements) {
        if (result.length >= 4) break;
        if (!result.find(a => a.id === announcement.id)) {
          result.push(announcement);
        }
      }
    }
  ...
  const paginatedAnnouncements = getDashboardAnnouncements();
  
  // Pagination logic
  const totalPages = announcementFilter === 'all' ? 1 : Math.ceil(filteredAnnouncements.length / itemsPerPage);
  ...
  return (
    <div className="bg-white content-stretch flex flex-col items-start relative rounded-tl-[16px] w-full overflow-hidden min-h-full">
      
      {/* Offboarding Banner - Paling Atas */}
      {isUserOffboarding && (
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: '16px' }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <p style={{
                  ...
                  Anda memiliki tugas offboarding yang perlu diselesaikan sebelum hari terakhir bekerja. Klik untuk melihat detail dan menyelesaikan checklist.
                </p>
              </div>
              <div style={{ flexShrink: 0 }}>
                <svg width="20" height="20" fill="none" viewBox="0 0 24 24" stroke="#92400E">
      ...
      <div className="bg-white relative shrink-0 w-full">
        <div className="flex flex-col items-end justify-center h-full w-full">
          <div className="content-stretch flex flex-col gap-[24px] items-end justify-center p-[24px] relative w-full">
            <div className="content-stretch flex gap-[24px] items-center relative shrink-0 w-full h-[80vh]">
              <BannerCarousel />
      ...
      {/* Onboarding Checklist Section - Above Employee Survey */}
      {onboardingContext?.newEmployeeChecklistItems && 
       onboardingContext.newEmployeeChecklistItems.length > 0 && 
       onboardingContext.showNewEmployeeChecklist && (
        <div style={{ padding: '0 24px' }}>
          <OnboardingDashboardSection
            items={onboardingContext.newEmployeeChecklistItems}
            onItemComplete={onboardingContext.completeChecklistItem}
                    ...
                    <div className="overflow-x-auto flex gap-[24px] items-center relative shrink-0 w-full">
                      ...
                      <div className="flex flex-row items-center self-stretch">
                        <div className="bg-white content-stretch flex gap-[24px] h-full items-start overflow-clip p-[24px] relative rounded-[16px] shadow-[0px_4px_8px_-2px_rgba(10,13,18,0.1),0px_2px_4px_-2px_rgba(10,13,18,0.06)] shrink-0" data-name="Employee Card HBD">
                          ...
                          <BackgroundImage1 text="Binavia Wardhani" text1="HC Strategy Senior Officer" text2="Head Office" />
                        </div>
                      </div>
                      <div className="flex flex-row items-center self-stretch">
      ...
      <ExternalSystemsBackgroundImage>
        ...
        <div className="relative shrink-0 w-full" data-name="Container" style={{
          display: 'grid',
          ...
          {/* Additional systems - shown when "Lihat Semua Sistem" is clicked */}
          {showAllSystems && (
            <>
            ...
            <BackgroundImage18>
              <div className="content-stretch flex h-[40px] items-start justify-between relative shrink-0 w-full" data-name="Container">
                <div className="bg-[#FEF3C7] relative rounded-[8px] shrink-0 size-[40px]" data-name="Container">
                  <div className="bg-clip-padding border-0 border-[transparent] border-solid content-stretch flex items-center justify-center overflow-clip p-px relative rounded-[inherit] size-full">
                    <div className="relative shrink-0 size-[40px] flex items-center justify-center">
                      <svg className="block size-[24px]" fill="none" viewBox="0 0 24 24" stroke="#F59E0B" strokeWidth="2">
              ...
              <div className="content-stretch flex flex-col gap-[4px] h-[44.5px] items-start relative shrink-0 w-full" data-name="Container">
                <HeadingBackgroundImageAndText1 text="Learning Management System" />
                <BackgroundImageAndText3 text="Platform pembelajaran online, training, dan development karyawan" />
              </div>
              <ButtonBackgroundImageAndText text="Buka LMS Portal" />
            </BackgroundImage18>
          </BackgroundImage7>
          ...
          <BackgroundImage7>
            <BackgroundImage18>
              <div className="content-stretch flex h-[40px] items-start justify-between relative shrink-0 w-full" data-name="Container">
                <div className="bg-[#D1FAE5] relative rounded-[8px] shrink-0 size-[40px]" data-name="Container">
                  <div className="bg-clip-padding border-0 border-[transparent] border-solid content-stretch flex items-center justify-center overflow-clip p-px relative rounded-[inherit] size-full">
                    <div className="relative shrink-0 size-[40px] flex items-center justify-center">
                      <svg className="block size-[24px]" fill="none" viewBox="0 0 24 24" stroke="#10B981" strokeWidth="2">
              ...
              <div className="content-stretch flex flex-col gap-[4px] h-[44.5px] items-start relative shrink-0 w-full" data-name="Container">
                <HeadingBackgroundImageAndText1 text="Payroll Management" />
                <BackgroundImageAndText3 text="Sistem penggajian, tunjangan, dan kompensasi karyawan" />
              </div>
              <ButtonBackgroundImageAndText text="Buka Payroll System" />
            </BackgroundImage18>
          </BackgroundImage7>
            ...
            <BackgroundImage18>
              <div className="content-stretch flex h-[40px] items-start justify-between relative shrink-0 w-full" data-name="Container">
                <div className="bg-[#DBEAFE] relative rounded-[8px] shrink-0 size-[40px]" data-name="Container">
                  <div className="bg-clip-padding border-0 border-[transparent] border-solid content-stretch flex items-center justify-center overflow-clip p-px relative rounded-[inherit] size-full">
                    <div className="relative shrink-0 size-[40px] flex items-center justify-center">
                      <svg className="block size-[24px]" fill="none" viewBox="0 0 24 24" stroke="#3B82F6" strokeWidth="2">
              ...
              <div className="content-stretch flex flex-col gap-[4px] h-[44.5px] items-start relative shrink-0 w-full" data-name="Container">
                <HeadingBackgroundImageAndText1 text="Time & Attendance" />
                <BackgroundImageAndText3 text="Sistem absensi, kehadiran, dan manajemen waktu kerja" />
              </div>
              <ButtonBackgroundImageAndText text="Buka Time & Attendance" />
            </BackgroundImage18>
          </BackgroundImage7>
            </>
          )}
        ...
        {/* Button Lihat Semua Sistem - Moved Below Cards */}
        {!showAllSystems && (
      ...
      <AnnouncementSectionBackgroundImage>
        <div className="content-stretch flex flex-col gap-[24px] items-start relative shrink-0 w-full" data-name="Pengumuman dan Informasi Penting">
          ...
          <div className="flex flex-col gap-4 w-full">
            ...
            <div className="flex items-center gap-2">
              {[
                ...
                <button
                  key={filter.value}
                  onClick={() => setAnnouncementFilter(filter.value as any)}
                  className="px-4 py-2 rounded-lg font-['Inter:Medium',sans-serif] font-medium text-[12px] transition-all"
                  style={{
                    backgroundColor: announcementFilter === filter.value ? '#00858A' : 'white',
                    color: announcementFilter === filter.value ? 'white' : '#717680',
                    border: `1px solid ${announcementFilter === filter.value ? '#00858A' : 'var(--border)'}`,
                  }}
                >
                  {filter.label}
                </button>
          ...
          {/* Conditional Rendering: List View or Carousel View */}
          {announcementView === "carousel" ? (
            /* Carousel View */
            <div style={{ marginTop: "24px" }}>
              <AnnouncementCarousel 
                announcements={announcements}
                onImageClick={(url) => setPreviewImage({ url, alt: "Announcement" })}
              />
            </div>
          ) : (
            /* List View - With Pagination using AnnouncementCard */
            <div style={{ padding: '0', width: '100%' }}>
              <div className="flex flex-col gap-4 w-full">
                {paginatedAnnouncements.map((announcement) => (
                  ...
                  >
                    <AnnouncementCard
                      category={announcement.iconType === 'learning' ? 'Learning' : 
                               announcement.iconType === 'talent' ? 'Talent' : 
                               announcement.iconType === 'performance' ? 'Performance' : 
                               announcement.iconType === 'ai' ? 'AI & Innovation' : 'General'}
                      date={new Date(announcement.timestamp).toLocaleDateString('id-ID', { 
                        day: 'numeric', 
                        month: 'long', 
                        year: 'numeric' 
                      })}
                      ...
                      ctaText={announcement.linkAttachment ? "Lihat Selengkapnya" : ""}
                      onCtaClick={announcement.linkAttachment ? (e?: React.MouseEvent) => {
                        e?.stopPropagation();
                        window.open(announcement.linkAttachment, '_blank');
                      } : undefined}
                      imageUrl={announcement.imageUrl}
                      iconType={announcement.iconType}
                      categoryColor={announcement.categoryColor}
                      onImageClick={(url, e?: React.MouseEvent) => {
                        e?.stopPropagation();
                        setPreviewImage({ url, alt: announcement.title });
                      }}
                    />
                  </div>
                ))}
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-center gap-2 mt-6">
                  ...
                  >
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#414651" strokeWidth="2">
          ...
          {/* Who's Off Section - After Announcements */}
          <div className="relative shrink-0 w-full bg-[#F8F9FC] rounded-[16px]">
            <div className="size-full">
              <div className="content-stretch flex flex-col gap-[24px] items-start p-[24px] relative w-full">
                <div className="content-stretch flex items-center justify-between relative shrink-0 w-full" data-name="Container">
                  <p className="font-['Inter:Semi_Bold',sans-serif] font-semibold leading-[30px] not-italic relative shrink-0 text-[#414651] text-[20px] text-nowrap whitespace-pre">Who's Off</p>
                  <select 
                    className="px-4 py-2 rounded-lg border border-[var(--border)] bg-[var(--card)] text-[var(--foreground)]"
                    style={{ 
                      fontSize: '14px',
                      fontFamily: 'Inter',
                      fontWeight: 500,
                ...
                {/* Employee Off List */}
                <div className="bg-white relative rounded-[16px] shadow-[0px_4px_8px_-2px_rgba(10,13,18,0.1),0px_2px_4px_-2px_rgba(10,13,18,0.06)] shrink-0 w-full p-[24px]">
                  <div className="flex flex-col gap-[16px] w-full">
                    {/* Employee 1 */}
                    <div className="flex items-center gap-[16px] pb-[16px] border-b border-[var(--border)]">
                      <div className="relative rounded-full shrink-0 size-[48px]">
                        <img 
                          ...
                          src={imgFrame1000001779}
                        />
                      </div>
                      <div className="flex-1">
                        <p className="font-['Inter:Semi_Bold',sans-serif] font-semibold text-[16px] text-[#414651] leading-[24px]">
                          Anisa Putri Maharani
                        </p>
                        <p className="font-['Inter:Regular',sans-serif] font-normal text-[14px] text-[#717680] leading-[20px]">
                          HR Business Partner - Human Capital
                        </p>
                      </div>
                      <div className="flex flex-col items-end gap-[4px]">
                        <div className="px-3 py-1 rounded-full bg-[rgba(0,133,138,0.1)] border border-[rgba(0,133,138,0.2)]">
                          <p className="font-['Inter:Medium',sans-serif] font-medium text-[12px] text-[#00858A] leading-[16px]">
                            Annual Leave
                          </p>
                        </div>
                        <p className="font-['Inter:Regular',sans-serif] font-normal text-[12px] text-[#717680] leading-[16px]">
                          21 Jan 2026
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-[16px] pb-[16px] border-b border-[var(--border)]">
                      ...
                      <div className="flex flex-col items-end gap-[4px]">
                        <div className="px-3 py-1 rounded-full bg-[rgba(251,191,36,0.1)] border border-[rgba(251,191,36,0.2)]">
                          <p className="font-['Inter:Medium',sans-serif] font-medium text-[12px] text-[#FBBF24] leading-[16px]">
                            Sick Leave
                          </p>
                        </div>
                        <p className="font-['Inter:Regular',sans-serif] font-normal text-[12px] text-[#717680] leading-[16px]">
                    ...
                    <div className="flex items-center gap-[16px] pb-[16px] border-b border-[var(--border)]">
                      ...
                      <div className="flex flex-col items-end gap-[4px]">
                        <div className="px-3 py-1 rounded-full bg-[rgba(139,92,246,0.1)] border border-[rgba(139,92,246,0.2)]">
                          <p className="font-['Inter:Medium',sans-serif] font-medium text-[12px] text-[#8B5CF6] leading-[16px]">
                            Maternity Leave
                          </p>
                        </div>
                        <p className="font-['Inter:Regular',sans-serif] font-normal text-[12px] text-[#717680] leading-[16px]">
                    ...
                    <div className="flex items-center gap-[16px]">
                      <div className="relative rounded-full shrink-0 size-[48px] bg-[#E5E7EB] flex items-center justify-center">
                        <p className="font-['Inter:Semi_Bold',sans-serif] font-semibold text-[20px] text-[#6B7280]">
                          DP
                        </p>
                      </div>
                      <div className="flex-1">
      ...
      {/* VIA (Virtual Injourney Assistant) - Right Side */}
      <AIChatbot />
