  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      type: 'bot',
      content: 'Halo! Saya Rinjani AI Assistant. Ada yang bisa saya bantu? Saya dapat membantu Anda dengan:\n\n• HC Policy & Prosedur\n• Informasi KPI\n• Data Talent\n• FAQ Umum',
      timestamp: new Date()
    }
  ]);
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (isOpen && chatWindowRef.current && !chatWindowRef.current.contains(event.target as Node)) {
        const floatingButton = document.querySelector('[aria-label="Open AI Chatbot"]');
