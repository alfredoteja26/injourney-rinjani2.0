...
function collectFigmaAssetAliases() {
  ...
  for (const packageName of packageNames) {
    const assetsDir = path.resolve(packagesRoot, packageName, "src/assets");
    ...
    for (const assetName of fs.readdirSync(assetsDir)) {
      ...
      if (!aliases[`figma:asset/${assetName}`]) {
        aliases[`figma:asset/${assetName}`] = assetPath;
      }
    }
  }
